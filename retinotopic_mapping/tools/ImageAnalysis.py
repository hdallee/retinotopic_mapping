...
def zscore(A):
    ...
    if np.isnan(A).any():
        return (A - np.nanmean(A.flatten())) / np.nanstd(A.flatten())
    else:
        A = A.astype(np.float); return (A - np.mean(A.flatten())) / np.std(A.flatten())
...
def generate_rectangle_mask(shape, center, width, height, isplot=False):
    ...
    mask = np.zeros(shape);
    mask[:] = np.nan
    mask[int(round(center[0] - height / 2)):int(round(center[0] + height / 2)),
...
def get_area_edges(img,
    ...
    imgPatch[imgMedianFiltered < areaThr[0]] = areaThr[0];
    imgPatch[imgMedianFiltered > areaThr[1]] = areaThr[1]
    ...
    edgesThr[edgesF < thr] = 0;
    edgesThr[edgesF >= thr] = 1

    edgesThin = sm.skeletonize(edgesThr)

    edgesThin = remove_small_patches(edgesThin, lengthThr)
    if borderWidth > 1:
        edgesThick = ni.binary_dilation(edgesThin, iterations=borderWidth - 1)
    else:
        edgesThick = edgesThin
    ...
    if isPlot:
        ...
        ax[0, 0].axis('off')
        ax[0, 1].imshow(imgFlat, cmap='gray');
        ax[0, 1].set_title('flattened image');
        ax[0, 1].axis('off')
        ax[0, 2].imshow(imgPatch, cmap='gray');
        ax[0, 2].set_title('image for edge detection');
        ax[0, 2].axis('off')
        ax[0, 3].imshow(firstEdgeSet, cmap='gray');
        ax[0, 3].set_title('first edge set');
        ax[0, 3].axis('off')
        ax[0, 4].imshow(lastEdgeSet, cmap='gray');
        ax[0, 4].set_title('last edge set');
        ax[0, 4].axis('off')
        ax[1, 0].imshow(edgesF, cmap='hot');
        ax[1, 0].set_title('filtered edges sum');
        ax[1, 0].axis('off')
        ax[1, 1].imshow(edgesThr, cmap='gray');
        ax[1, 1].set_title('binary thresholded edges');
        ax[1, 1].axis('off')
        ax[1, 2].imshow(edgesThick, cmap='gray');
        ax[1, 2].set_title('binary edges');
        ax[1, 2].axis('off')
        ax[1, 3].imshow(img, cmap='gray');
        ax[1, 3].imshow(displayEdges);
        ax[1, 3].set_title('original image with edges');
        ax[1, 3].axis('off')
        ax[1, 4].imshow(imgPatch, cmap='gray');
        ax[1, 4].imshow(displayEdges);
        ax[1, 4].set_title('blurred image with edges');
        ax[1, 4].axis('off')
        plt.tight_layout()
        return edgesThick.astype(np.bool), f

    else:
        return edgesThick.astype(np.bool)
...
class ROI(object):
    ...
    def plot_binary_mask(self, plotAxis=None, color='#ff0000', alpha=1):
        ...
        displayImg = pt.binary_2_rgba(mask, foregroundColor=color, backgroundColor='#000000',
                                      foregroundAlpha=int(alpha * 255), backgroundAlpha=0)
        if plotAxis is None: f = plt.figure();plotAxis = f.add_subplot(111);plotAxis.imshow(displayImg,
    ...
    def to_h5_group(self, h5Group):
        ...
        if self.pixelSizeX is None:
            h5Group.attrs['pixelSize'] = 'None'
        else:
            h5Group.attrs['pixelSize'] = [self.pixelSizeY, self.pixelSizeX]
        if self.pixelSizeUnit is None:
            h5Group.attrs['pixelSizeUnit'] = 'None'
        else:
            h5Group.attrs['pixelSizeUnit'] = self.pixelSizeUnit

        dataDict = dict(self.__dict__)
        _ = dataDict.pop('dimension');
        _ = dataDict.pop('pixelSizeX');
        _ = dataDict.pop('pixelSizeY');
        _ = dataDict.pop('pixelSizeUnit')
        for key, value in dataDict.items():
            if value is None:
                h5Group.create_dataset(key, data='None')
            else:
