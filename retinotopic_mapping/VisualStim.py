...
# from ../retinotopic_mapping-Git_master.retinotopic_mapping.tools import FileTools as ft
...
try:
    import toolbox.IO.nidaq as iodaq
...
class Monitor(object):
    ...
    def plot_map(self):
        ...
        levels1 = list(
            range(int(np.floor(self.linCorX.min() / 10) * 10), int((np.ceil(self.linCorX.max() / 10) + 1) * 10), 10))
        ...
        levels2 = list(
            range(int(np.floor(self.linCorY.min() / 10) * 10), int((np.ceil(self.linCorY.max() / 10) + 1) * 10), 10))
        ...
        levels3 = list(
            range(int(np.floor(self.degCorX.min() / 10) * 10), int((np.ceil(self.degCorX.max() / 10) + 1) * 10), 10))
        ...
        levels4 = list(
            range(int(np.floor(self.degCorY.min() / 10) * 10), int((np.ceil(self.degCorY.max() / 10) + 1) * 10), 10))
...
class Stim(object):
    ...
    def generate_frames(self):
        ...
        print(
            'This function should return a list of tuples, each tuple represents a single frame of the stimulus and contains all the information to recreate the frame.')
...
class KSstim(Stim):
    ...
    def __init__(self,
        ...
        super(KSstim, self).__init__(monitor=monitor, indicator=indicator, coordinate=coordinate, background=background,
                                     preGapDur=preGapDur, postGapDur=postGapDur)
...
class NoiseKSstim(Stim):
    ...
    def __init__(self,
        ...
        super(NoiseKSstim, self).__init__(monitor=monitor, indicator=indicator, background=background,
    ...
    def generate_sweeps(self):
        ...
        if direction == "B2U":
            stepY = np.arange(minY - edgeWidth - sweepWidth / 2, maxY + edgeWidth + stepWidth + sweepWidth / 2,
                              stepWidth)
        elif direction == "U2B":
            stepY = np.arange(minY - edgeWidth - sweepWidth / 2, maxY + edgeWidth + stepWidth + sweepWidth / 2,
                              stepWidth)[::-1]
            ...
            stepX = np.arange(minX - edgeWidth - sweepWidth / 2, maxX + edgeWidth + stepWidth + sweepWidth / 2,
                              stepWidth)
        elif direction == "R2L":
            stepX = np.arange(minX - edgeWidth - sweepWidth / 2, maxX + edgeWidth + stepWidth + sweepWidth / 2,
                              stepWidth)[::-1]
...
class ObliqueKSstim(Stim):
    ...
    def __init__(self,
        ...
        super(ObliqueKSstim, self).__init__(monitor=monitor, indicator=indicator, background=background,
                                            coordinate=coordinate, preGapDur=preGapDur, postGapDur=postGapDur)
    ...
    def generate_sweeps(self):
        ...
        map_x_r = map_rotated[0].reshape(mapX.shape);
        map_y_r = map_rotated[1].reshape(mapY.shape)
...
class FlashingNoise(Stim):
    ...
    def __init__(self,
        ...
        super(FlashingNoise, self).__init__(monitor=monitor, indicator=indicator, background=background,
                                            coordinate=coordinate, preGapDur=preGapDur, postGapDur=postGapDur)
...
class GaussianNoise(Stim):
    ...
    def __init__(self,
        ...
        super(GaussianNoise, self).__init__(monitor=monitor, indicator=indicator, background=background,
                                            coordinate=coordinate, preGapDur=preGapDur, postGapDur=postGapDur)
...
class SparseNoise(Stim):
    ...
    def __init__(self,
        ...
        super(SparseNoise, self).__init__(monitor=monitor, indicator=indicator, background=background,
                                          coordinate=coordinate, preGapDur=preGapDur, postGapDur=postGapDur)
    ...
    def generate_movie(self):
        ...
        if self.coordinate == 'degree':
            corX = self.monitor.degCorX;corY = self.monitor.degCorY
        elif self.coordinate == 'linear':
            corX = self.monitor.linCorX;corY = self.monitor.linCorY
        ...
        fullSequence = np.ones((len(self.frames), self.monitor.degCorX.shape[0], self.monitor.degCorX.shape[1]),
...
class DriftingGratingCircle(Stim):
    ...
    def __init__(self,
        ...
        super(DriftingGratingCircle, self).__init__(monitor=monitor, indicator=indicator, background=background,
                                                    coordinate=coordinate, preGapDur=preGapDur, postGapDur=postGapDur)
    ...
    def generate_frames(self):
        ...
        for i in range(self.iteration):
            if i == 0:  # very first block
                ...
                frames += [[0, None, None, None, None, None, None, None, -1.] for ind in
                           range(int(self.midGapDur * self.monitor.refreshRate))]
...
class KSstimAllDir(object):
    ...
    def __init__(self,
                 ...
                 coordinate='degree',  # 'degree' or 'linear'
                 background=0.,  # back ground color [-1,1]
                 squareSize=10,  # size of flickering square
                 squareCenter=(0, 0),  # coordinate of center point
                 flickerFrame=6,
                 sweepWidth=20.,  # width of sweeps (unit same as Map, cm or deg)
                 stepWidth=0.15,  # width of steps (unit same as Map, cm or deg)
    ...
    def generate_movie(self):
        ...
        from .tools.introspect import hash_variables
        param_hash = hash_variables(
            (self.background, self.coordinate, self.squareSize, self.flickerFrame, self.sweepWidth, self.stepWidth,
             self.sweepFrame, self.iteration, self.preGapDur, self.postGapDur,), digest='hex')
        cachename = os.path.join(basedir, 'stimulus_videos/{0}.hdf5'.format(param_hash))
        if os.path.exists(cachename):
            with closing(Hdf5io(cachename, filelocking=False)) as fh:
                mov = fh.findvar('mov')
                directions = fh.findvar('directions')
                if directions is not None:
                    dictB2U, dictU2B, dictL2R, dictR2L = pickle.loads(directions)
        else:
            ...
            with closing(Hdf5io(cachename, filelocking=False)) as fh:
                fh.mov = mov
                fh.directions = pickle.dumps([dictB2U, dictU2B, dictL2R, dictR2L], protocol=2)
                fh.save(['mov', 'directions'])
...
class DisplaySequence(object):
    ...
    def __init__(self,
                 ...
                 # should be one of "NegativeEdge", "PositiveEdge", "HighLevel", or "LowLevel"
                 displayScreen=0,
    ...
    def trigger_display(self, fullscr=None):
        ...
        self.window = visual.Window(size=resolution, monitor=self.psychopyMonitor, fullscr=fullscr,
                                    screen=self.displayScreen,
                                    color=self.initialBackgroundColor)
        self.window.recordFrameIntervals = True
        stim = visual.ImageStim(self.window, size=(2, 2), interpolate=self.isInterpolate)
        # ----------------------------setup psychopy window and stimulus------------------------------------------------
        ...
        # display
        self._display(self.window, stim)  # display sequence
        ...
        if event == 'LowLevel':
            ...
            while lastTTL != 0 and self.keepDisplay:
                ...
                if self.keepDisplay:
                    ...
                    triggerTask.StopTask(); print(
                        'Manual stop signal detected during waiting period. Stop the program.'); return False
            ...
            while self.keepDisplay:
                ...
                if (lastTTL == 1) and (currentTTL == 0):
                    ...
                    lastTTL = int(currentTTL);self._update_display_status()
            else:
                triggerTask.StopTask(); print(
                    'Manual stop signal detected during waiting period. Stop the program.');return False
    ...
    def _display(self, window, stim):
        ...
        while self.keepDisplay and i < (singleRunFrames * self.displayIteration) and not self.interrupt.is_set():
        ...
        # timeStamp.append(time.time()-self.startTime)
        stopTime = time.time()
        window.close()
        ...
        self.displayLength = stopTime - self.startTime
    ...
    def _update_display_status(self):
        ...
        if self.keepDisplay is None: raise LookupError(
            'self.keepDisplay should start as True for updating display status')
...
if __name__ == "__main__":
    ...
    experiment_type = 'long'  # set to 'overview' to acquire only 1 repetition while the camera is focused on the surface of the brain. otherwise 'long'
    ...
    mon = Monitor(resolution=(768, 1360), dis=13.0, monWcm=stimulus_parameters['monitor_width_cm'],
                  monHcm=stimulus_parameters['monitor_height_cm'],
                  C2Tcm=stimulus_parameters['gaze_center_to_monitor_top_edge_cm'],
                  C2Acm=stimulus_parameters['gaze_center_to_monitor_anterior_edge_cm'],
                  monTilt=16.22, visualField='right', downSampleRate=8)  # 20

    indicator = Indicator(mon)
    KS_stim_all_dir = KSstimAllDir(mon, indicator, sweepWidth=stimulus_parameters['sweep_width'], squareSize=25,
                                   flickerFrame=6, iteration=1,
                                   stepWidth=stimulus_parameters['step_width'], preGapDur=3.,
                                   postGapDur=5.)  # stepWidth=0.15, # stepWidth*refreshRate=speed, i.e. 0.15 deg *60 = 9 deg/s
    ds = DisplaySequence(logdir=r'C:\data', displayScreen=1, backupdir=None,
                         displayIteration=stimulus_parameters['repeat_stimulus'],
                         isTriggered=False, isSyncPulse=False, interrupt=camera.camera_error_trigger)
    ...
    with closing(Hdf5io(camcfg.CAMERA['output'] if save_to_hdf5 else camcfg.CAMERA['output'] + '/timing.hdf5',
        ...
        output_file.stimulus_frame_timestamps = ds.timeStamp
        output_file.stimulusLog = pickle.dumps(ds.sequenceLog, protocol=2)
        output_file.saving_complete = True
        output_file.dropped_stimulus_frames = ds.window.nDroppedFrames
        output_file.save(['stimulus_frame_timestamps', 'stimulusLog', 'saving_complete'])
        output_file.saving_complete = True
    ...
    MPL.imshow(camera_mean, cmap=cm.gray)
    MPL.show()
